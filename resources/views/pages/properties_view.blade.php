...
<script>
    ...
    // Original search functionality with error handling
    document.addEventListener('DOMContentLoaded', function() {
        try {
            const searchFormInput = document.getElementById('searchInput');
            if (searchFormInput) {
                document.getElementById('searchForm').addEventListener('submit', function(event) {
                    searchFormInput.value = searchFormInput.value.replace(/\s+/g, '+');
                });
            }
        } catch (error) {
            console.log('Search form input not found, skipping space replacement');
        }
    });
</script>
    ...
    <div class="container">
        ...
        @else
            <div class="no-results">
                ...
                <h4>Please click <a href="{{ url('property/' . $url_type) }}">here</a> to see all
